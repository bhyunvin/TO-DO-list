...
export class FileUploadErrorService {
  ...
  getUserFriendlyMessage(error: FileValidationError): string {
    const baseMessage = FILE_VALIDATION_MESSAGES[error.errorCode] || error.errorMessage;
    
    switch (error.errorCode) {
      case FILE_VALIDATION_ERRORS.FILE_TOO_LARGE:
        const sizeInfo = error.fileSize ? ` (${this.formatFileSize(error.fileSize)})` : '';
   ...
   */
  private groupErrorsByType(errors: FileValidationError[]): Record<string, FileValidationError[]> {
