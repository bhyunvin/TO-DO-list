...
@Controller('todo')
@UseGuards(AuthGuard) // 이 컨트롤러의 모든 라우트에 AuthGuard를 적용합니다.
export class TodoController {
  constructor(private readonly todoService: TodoService) {}

  // 새로운 ToDo 항목을 생성합니다.
  @Post()
  create(@Session() session: Record<string, any>, @Body() createTodoDto: CreateTodoDto) {
  ...
  @Get()
  findAll(@Session() session: Record<string, any>, @Query('date') date: string) {
    const userSeq = session.user.userSeq;
  ...
  remove(@Session() session: Record<string, any>, @Body() deleteTodoDto: DeleteTodoDto) {
    const userSeq = session.user.userSeq;
