...
function CreateTodoForm(props) {
    ...
    function handleChange(e) {
        ...
        switch ( thisName ) {
            case 'TODO_CONTENT': setTodoContent(e.target.value); break;
            case 'TODO_NOTE': setTodoNote(e.target.value); break;
            case 'TODO_FILES': const selectedFiles = Array.from(e.target.files); setTodoFiles(selectedFiles); break;
            default: break;
        }
    }
...
function TodoList(props) {
    ...
    function handleWholeCheckbox(e) {
        ...
        Array.from( document.getElementsByClassName('todo-checkbox') )
        .forEach(function(todoCheckboxElement) {
    ...
    return (
        <table className="todo-list">
            ...
            <thead>
                <th>
                    <input type="checkbox" className="form-check-input" onClick={handleWholeCheckbox}></input>
                </th>
                <th>번호</th>
                <th>내용</th>
                <th>완료일시</th>
            ...
            {todos.length > 0 ? (
                todos.map(function(todo, index) {
                    return  <tr id={todo.TODO_SEQ} key={todo.TODO_SEQ}>
                                <td>
                                    <input type="checkbox" className="form-check-input todo-checkbox" checked=""></input>
                                </td>
                                <td>{index + 1}</td>
...
function TodoContainer() {
    ...
    function handleModifyTodo() {
        ...
        const todoCheckboxElements = Array.from( document.getElementsByClassName('todo-checkbox') )
                                        .filter(function(todoCheckbox) { return todoCheckbox.checked; })
                                        .map(function(checkedElement) { return checkedElement.id; });
    ...
    return (
        <div className="todo-container">
            <h2>TO-DO 리스트</h2>
            <button className={(isCreating || isEditing) ? 'btn btn-secondary mb-3' : 'btn btn-primary mb-3'} onClick={handleToggleCreate}>
                {(isCreating || isEditing) ? '취소' : '신규'}
            </button>
            {(!isCreating && !isEditing) && <button className="btn btn-secondary mb-3 mr-3" onClick={getUncompleteTodoList}>완료되지 않은 할 일 불러오기</button>}
            {(!isCreating && !isEditing) && <button className="btn btn-secondary mb-3 mr-3" onClick={handleModifyTodo}>수정</button>}
            {!isCreating && <button className="btn btn-danger mb-3 mr-3" onClick={handleDeleteTodo}>삭제</button>}
            {isCreating ? (
                <CreateTodoForm onAddTodo={handleAddTodo} onCancel={handleToggleCreate} />
            ) : (
