...
const todoService = {
  ...
  async deleteAttachment(todoSeq: number | string, fileNo: number | string) {
    ...
    const { error } = await api.todo({ todoId: Number(todoSeq) }).file({ fileNo: Number(fileNo) }).delete();
    if (error) throw new Error(String(error.value));
  },
  ...
  async updateTodo(todoSeq: number | string, data: any) {
    ...
    // PATCH /:id
    const { data: result, error } = await api.todo({ id: Number(todoSeq) }).patch(payload);
    if (error) throw new Error(String(error.value));
    return result;
  },
